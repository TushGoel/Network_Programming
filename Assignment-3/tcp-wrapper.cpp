...
TcpWrapper::TcpWrapper(int port_num):m_port{port_num},m_ip{""},m_exit{false}, m_l_sock_fd{-1}, m_sock_fd{-1}
...
// - call the function run()
{
  ...
  else
  {
    if(!optionsAndBind())
    {
      std::cout << "Error: Unable to call the function optionsAndBind" << std::endl;
      close(m_l_sock_fd);
      return;
    }
    run();
...
TcpWrapper::run() 
{
  ...
  if(acceptClient()) 
  {
    ...
    while(!m_exit)
    {
      ...
      if(data_size < 0)
      {
        std::cout << "Error: Unable to receive the data" << std::endl;
        m_exit = true;
        break;
      }
      else if(data_size == 0)
      {
        std::cout << "Error: Either Client socket is closed or TCP receive buffer is full" << std::endl;
        m_exit = true;
        break;
      }
      ...
      memset(buf, 0, C_MTU);
      char acknowl[] = "Packet ACK\n";
      int acknowl_size = sizeof(acknowl);
      int sent_size = sendData(acknowl, acknowl_size);
      if (sent_size < 0)
      {
        m_exit = true;
        break;
...
TcpWrapper::acceptClient() 
{
  ...
  if (m_sock_fd < 0)
  {
    std::cout << "Error: Unable to accept" << std::endl;
    if (m_l_sock_fd >= 0)
    {
      close(m_l_sock_fd);
    }
  ...
  else
  {
    ...
    std::cout << " Connection from IP: " << m_ip << std::endl;
    return true; 
  }
}
